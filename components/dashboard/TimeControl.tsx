...
const PILL =
  'inline-flex min-h-11 items-center justify-center rounded-full border px-4 text-[10px] font-semibold tracking-[0.14em] whitespace-nowrap uppercase transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-300 disabled:cursor-default disabled:opacity-35';

const QUIET_PILL = `${PILL} border-slate-400/15 bg-[rgba(8,7,29,0.72)] text-slate-400 enabled:hover:border-cyan-300/35 enabled:hover:text-slate-200`;
...
const END_LABEL = 'shrink-0 text-[9px] font-semibold tracking-[0.14em] text-slate-600';
    ...
    >
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <div className="ml-auto flex items-center gap-2.5 font-mono text-[11px] tracking-[0.06em]">
          {/* Announced because it only ever changes in answer to a press. */}
          <span aria-live="polite">
            <span className={live ? 'text-cyan-200' : 'text-violet-200'}>
              {formatOffset(offsetMs)}
            </span>
          </span>
          <span className="clock-divider" aria-hidden="true" />
          {/* Deliberately outside the live region: it reissues every second,
              and a screen reader reading the clock aloud at 1Hz is unusable. */}
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => onOffsetChange(offsetMs - OFFSET_STEP_MS)}
          ...
          <button
            ...
            disabled={live}
            onClick={onReset}
            className={live ? QUIET_PILL : RESET_PILL}
          >
            NOW
          </button>
        </div>
