...
- the first-load byte budget, via `node scripts/check-bundle-budget.mjs` in the verification matrix after the build — a deterministic number, unlike the timings in `docs/perf/`, which are reported and never gated (ADR 0009)
