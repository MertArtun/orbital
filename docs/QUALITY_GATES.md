...
- the first-load byte budget, via `node scripts/check-bundle-budget.mjs` after the build, in the local verification matrix and in CI's `build` job — a deterministic number, unlike the timings in `docs/perf/`, which are reported and never gated (ADR 0009)
